...
namespace WebAPI_KTOnline.Controllers
{
    public class LoginController : ApiController
    {
        ...
        public GiaoVien Post_login_ADmin([FromBody] GiaoVien gv)
        {
            SqlConnection conn = DataProvider.Connect();
            conn.Open();
            String sQuery = string.Format("SELECT * FROM GiangVien where MaGV = '{0}' and Passsword = '{1}' and isAdmin = 'Y'", gv.maGiaoVien, StringProc.MD5Hash(gv.matKhau));
            SqlCommand comm = new SqlCommand(sQuery, conn);
            int count = 0;
            SqlDataReader dr = comm.ExecuteReader();
            while (dr.Read())
